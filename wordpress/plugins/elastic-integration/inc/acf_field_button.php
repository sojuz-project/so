...
class acf_field_button extends acf_field {
	...
	*/
	...
	function render_field( $field ) {
		...
		// Input.
		$input_attrs = array();
		foreach( array( 'type', 'id', 'class', 'name', 'value', 'readonly', 'disabled' ) as $k ) {
			if( isset($field[ $k ]) ) {
	...
	*/
    ...
    acf_render_field_setting( $field, array(
			'label'			=> __('Type','acf'),
			'instructions'	=> __('Defines the type of button','acf'),
      'type'			=> 'select',
      'choices'   => [
        'submit' => __('Submit', 'acf'),
        'reset' => __('Reset', 'acf'),
        'button' => __('Ordinary button', 'acf')
      ],
			'name'			=> 'type',
		));
...
// initialize
acf_register_field_type( 'acf_field_button' );
