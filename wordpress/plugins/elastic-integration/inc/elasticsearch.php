...
function elasticsearch($args, $endpoint, $method = 'GET') {
  ...
  $url                = "http://elasticsearch:9200/{$endpoint}";
...
function notify_index( $post_id, $post = NULL, $update = false ) {
  ...
  foreach ($tax as $t) {
    $taxonomy[$t->object_type[0]][] = $t->name;
  }
  ...
  foreach ($toIndex as $field) {
    switch ($field) {
      ...
      case"author":
        ...
        break;
      case"blocks":
        $data[$field] = parse_blocks($post->post_content);
        break;
      case"related":
	...
	$response = elasticsearch($data, $index.'/_doc/'.$post_id, 'PUT');
}
...
function remove_from_index($id) {
  elasticsearch(false, $index.'/_doc/'.$id.'/?pretty', 'DELETE');
}
